...
<html lang="fr">
    ...
    <body>
        <h1>Administration - Modérer les commentaires</h1>
        ...
        $req = $db->prepare('SELECT id, author, comment, DATE_FORMAT(comment_date, \'%d/%m/%Y à %Hh%imin%ss\') AS comment_date_fr FROM comments WHERE id_chapter = ? ORDER BY reporting DESC, comment_date DESC LIMIT ?, ?');
        ...
        while ($data = $req->fetch())
        {
            ...
            <em><a href="delete_comment.php?chapter=<?= $_GET['chapter']; ?>&amp;id=<?= $data['id']; ?>">Supprimer le commentaire</a></em>
            <em><a href="update_coms.php?chapter=<?= $_GET['chapter']; ?>&amp;id=<?= $data['id']; ?>">Modifier le commentaire</a></em>
            <?php
        ...
        <div class="news">
            <form action="post_comment.php?chapter=<?= $_GET['chapter']; ?>&amp;id=<?= $data['id']; ?>" method="POST">
                ...
                <button>
                    <input type="submit" value="Envoyer votre réponse au commentaire"/>
                </button>
            </form>
        </div>
        ...
        for ($i = 1 ; $i <= $pages_number ; $i++){
            if ($i == $current_page){
                ...
                echo '<span><a href="moderate_comments.php?chapter=' . $_GET['chapter'] . '&amp;page=' . $i . '">' . $i . '</a> </span>';
            }
        }
        echo ' </div>';
